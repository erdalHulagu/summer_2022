...
*/
    ...
    private static void testSonucu(char[][] answers, char[] keys) {
        ...
        for (int i = 0; i <answers.length; i++) {// herbir ogrenciye bakar  i-> ogrenci
            ...
            for (int j = 0; j <answers[i].length; j++) {// her bir ogrencinin notuna bakar j->cevaplar

                if(answers[i][j]==keys[j]){  // cevap anahtari ile ogrenci cevaplarini karsilastirma
                    sayac++;
                }
            }
            System.out.println(i+1+"nolu ogrencinin cevap sayisi : "+sayac);
        }
