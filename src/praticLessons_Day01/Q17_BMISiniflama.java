package praticLessons_Day01;

import java.util.Scanner;

public class Q17_BMISiniflama {
    /*
 *  Kullanicidan boyunu  CM  ve kilosunu kg alarak  BMI (VKE) hesaplayiniz
 *
BMI = kilo(kg) /(boy*boy)(m)
BMI <=20 oldukca zayifsiniz
20<BMI<=25 Normal sinirlardasiniz
25<BMI<=30 Sisman kategorisindesiniz
30<BMI ==> Obez grubundasiniz.

 */
    public static void main(String[] args) {
        Scanner scan=new Scanner(System.in);
        System.out.println("lutfen kiloniuzu giriniz kg");
        double kilo= scan.nextDouble();
        System.out.println("lutfen boyunuzu giriniz cm");
        double boy= scan.nextDouble()/100;
        double bmi=kilo/(boy*boy);
        if(bmi<=20){
            System.out.println("bmi indexiniz = "+bmi+"oldukca zayifsiniz");
        } else if (bmi<=25) {
            System.out.println("bmi indexiniz = "+bmi+"normal sinirlardasiniz");
        } else if (bmi<=30) {
            System.out.println("bmi indexiniz = "+bmi+"sisman kategoridesiniz");
        }else if(bmi>30){
            System.out.println("bmi indexiniz = "+bmi+"obezitesininz");
        }

    }
}
